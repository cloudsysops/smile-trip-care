...
import { ItineraryOutputSchema } from "@/lib/ai/schemas";
import type { OpsTasksOutput } from "@/lib/ai/schemas";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: lead, error: leadError } = await supabase
      .from("leads")
      .select("id, first_name, last_name, email, phone, country, package_slug")
    ...
    const { data: itineraryRows } = await supabase
      ...
      .limit(1);
    const itineraryMaybe = itineraryRows?.[0]?.content_json != null
      ? ItineraryOutputSchema.safeParse(itineraryRows[0].content_json)
    ...
    const input = {
      ...
      itinerary: itineraryMaybe.success ? itineraryMaybe.data : null,
    };

    const result = await runAgent<OpsTasksOutput>("ops-coordinator", input, { requestId });
    await saveLeadAI(parsed.data.lead_id, "ops", result.output as unknown as Record<string, unknown>);

    log.info("Ops tasks generated", { lead_id: parsed.data.lead_id });
    return NextResponse.json({ ops: result.output, request_id: requestId });
  } catch (err) {
    log.error("Ops route error", { err: String(err) });
    return NextResponse.json(
