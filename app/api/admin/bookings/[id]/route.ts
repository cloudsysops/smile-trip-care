...
export async function GET(_request: Request, { params }: Props) {
  ...
  try {
    ...
    log.error("Admin booking GET failed", { err: String(err) });
...
export async function PATCH(request: Request, { params }: Props) {
  ...
  try {
    ...
    const { data, error } = await updateBooking(parsedParams.data.id, parsed.data);
    if (error) {
      return NextResponse.json(
    ...
    log.error("Admin booking PATCH failed", { err: String(err) });
    return NextResponse.json(
      { error: "Internal server error", request_id: requestId },
      { status: 500 }
    );
