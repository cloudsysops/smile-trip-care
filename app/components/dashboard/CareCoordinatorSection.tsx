...
export default function CareCoordinatorSection({ coordinatorName }: Props) {
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 shadow-sm">
      <h3 className="text-sm font-semibold uppercase tracking-wider text-zinc-400">Care coordinator</h3>
      <p className="mt-1 text-sm text-zinc-400">Your dedicated point of contact</p>
      <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
        <div className="min-w-0 flex-1">
          ...
          {!coordinatorName?.trim() && (
            <p className="mt-2 text-sm text-zinc-400">{COORDINATOR_PLACEHOLDER}</p>
          )}
        </div>
      ...
      <div className="mt-6 rounded-lg border border-dashed border-zinc-700 bg-zinc-900/40 p-4 text-center">
        <p className="text-sm text-zinc-400">Prefer chat? Reach your coordinator on WhatsApp anytime.</p>
      </div>
    </section>
  );
}
