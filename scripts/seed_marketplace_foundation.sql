-- Marketplace Foundation Seed — Curated network (Clínica San Martín + specialists, experiences, packages, junctions).
-- Run after migration 0010_curated_network_enterprise.sql. Idempotent: uses ON CONFLICT for providers (slug), specialists (slug), experiences (slug), packages (slug), package_specialists, package_experiences.
-- See docs/MARKETPLACE_FOUNDATION.md for sample seeded entities and execution order.
